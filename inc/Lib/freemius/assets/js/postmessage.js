(function ($, undef) {
  ...
  global.FS.PostMessage = (function () {
    var _is_child = false,
      ...
      _callbacks = {},
      _base_url,
      _parent_url = decodeURIComponent(
        document.location.hash.replace(/^#/, "")
      ),
      _parent_subdomain = _parent_url.substring(
        0,
        _parent_url.indexOf(
          "/",
          "https://" === _parent_url.substring(0, "https://".length) ? 8 : 7
        )
      ),
      _init = function () {
      ...
      postScroll: function (iframe) {
        this.post(
          "scroll",
          {
            ...
            height:
              $window.height() -
              parseFloat($html.css("paddingTop")) -
              parseFloat($html.css("marginTop")),
          },
          iframe
        );
      },
