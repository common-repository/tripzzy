...
if ( ! class_exists( 'Tripzzy\Core\Emails\AdminBookingEmail' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class AdminBookingEmail extends EmailBase {
		...
		public static function email_subject() {
			return self::$settings['admin_booking_notification_subject'];
		}
		...
		public static function email_content() {
			...
			$content  = ! empty( $settings['admin_booking_notification_content'] ) ? $settings['admin_booking_notification_content'] : '';
			...
			<p style="line-height: 1.55; font-size: 14px;">Hello Admin,</p>
			<p style="line-height: 1.55; font-size: 14px;">You have received new bookings from %CUSTOMER_NAME%. Below are the booking and customer details:</p>
			<table>
		...
		public static function get_tags() {
			...
			return apply_filters( 'tripzzy_filter_admin_booking_email_tags', $tags );
		}
			...
			foreach ( $tag_keys as $tag_key ) {
				switch ( $tag_key ) {
					...
					case '%BOOKING_DETAILS%':
						$this->set_tag_value( $tag_key, Bookings::render_booking_details( $this->booking_id, true ) ); // Render wrapper method along with title.
						break;
