...
if ( ! class_exists( 'Tripzzy\Core\Helpers\TripInfos' ) ) {
	...
	class TripInfos {
		 ...
		 * @return void
		 */
		public static function render( $trip_id = 0, $has_return = false ) {
			if ( ! $trip_id ) {
			...
			$infos         = self::get( $trip_id ); // Saved In post meta.
			...
			if ( is_array( $infos ) && count( $infos ) > 0 ) {
				...
				ob_start();
				?>
				<div class="tripzzy-section" id="tripzzy-trip-infos-section">
							...
							<li>
								...
								<span class="info-values">
									...
									switch ( $type ) {
										...
										case 'text':
										case 'number':
											echo $info['value'] ? esc_html( $info['value'] ) : esc_html( 'N/A' );
											break;
										case 'textarea':
											echo $info['value'] ? wp_kses_post( $info['value'] ) : esc_html( 'N/A' );
											break;
									}
									?>
								</span>
