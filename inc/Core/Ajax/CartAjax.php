...
if ( ! class_exists( 'Tripzzy\Core\Ajax\CartAjax' ) ) {
	...
	class CartAjax {
		...
		protected function validate_cart_request( $cart_data ) {
			return ! ! $cart_data && isset( $cart_data['trip_id'] ) && isset( $cart_data['categories'] ) && is_array( $cart_data['categories'] ) && array_sum( $cart_data['categories'] ) > 0;
		}
		...
		protected function validate_item_qty( $cart_data ) {
			...
			return ! ! $cart_data && ! $has_negative_qty;
		}
		...
		protected function validate_cart( $cart_data ) {
			...
			if ( ! ! $cart_data ) {
				...
				if ( ! ( $total_qty >= (int) $min_people ) ) {
					$response['success'] = false;
					$response['message'] = ErrorMessage::get( 'min_cart_value_required', array( $min_people ) );
					return $response;
				}
